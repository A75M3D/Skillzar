...
class LanguageManager {
    constructor() {
        ...
        this.translations = {
            ar: {
                ...
                copyright: "جميع الحقوق محفوظة © 2025 Skillzoy Academy",
            ...
            en: {
                ...
                copyright: "All rights reserved © 2025 Skillzoy Academy",
    ...
    changeLanguage(lang) {
        ...
        showNotification(
            ...
            this.translate('language_changed_message'),
            'success'
        );
    }
...
class ProgressSystem {
    ...
    checkCourseCompletion(courseId) {
        ...
        if (isCompleted && !courseProgress.courseCompleted) {
            ...
            if (document.getElementById('course-details') && document.getElementById('course-details').style.display === 'block') {
...
class CertificateManager {
    ...
    displayCertificate(certificate) {
        ...
        const issueDateEl = document.getElementById('certificate-issue-date');
        
        if (studentNameEl) studentNameEl.textContent = certificate.studentName;
        if (courseNameEl) courseNameEl.textContent = certificate.courseTitle;
        ...
        if (dateEl) dateEl.textContent = `في تاريخ ${issueDate}`;
        if (idEl) idEl.textContent = `رقم الشهادة: ${certificate.id}`;
        if (issueDateEl) issueDateEl.textContent = `تاريخ الإصدار: ${issueDate}`;
        ...
        if (certificateContainer) {
            ...
            setTimeout(() => {
                certificateContainer.scrollTop = 0;
            }, 100);
        }
    }
    ...
    async downloadAsImage() {
        try {
            ...
            const studentName = document.getElementById('certificate-student-name')?.textContent || 'شهادة';
...
// ===== CategoryCarousel المفقود =====
class CategoryCarousel {
    constructor() {
        console.log('✅ CategoryCarousel initialized');
    }
    
    reset() {
        console.log('🔄 CategoryCarousel reset');
    }
}
...
// إعداد مستمعي الأحداث للتصنيفات
...
async function renderCourses(category = 'all') {
    ...
    container.innerHTML = coursesHTML || '<div class="empty-state">لا توجد دورات متاحة</div>';
    setupCourseEventListeners();
    
    // تحديث إحصائيات الدورات
    const coursesCountEl = document.getElementById('courses-count');
    if (coursesCountEl) {
        coursesCountEl.textContent = formatNumber(coursesData.length);
    }
}

// إعداد مستمعي الأحداث للدورات
...
async function renderCourseDetails(courseId) {
    ...
    // تحديل عناصر واجهة المستخدم
    const titleEl = document.getElementById('course-details-title');
    const descEl = document.getElementById('course-details-description');
    const durationEl = document.getElementById('course-total-duration');
    const levelEl = document.getElementById('course-level-text');
    const instructorEl = document.getElementById('course-instructor');
    const videosTextEl = document.getElementById('course-videos-text');
    
    if (titleEl) titleEl.textContent = safeTitle;
    if (descEl) descEl.textContent = safeDescription;
    if (durationEl) durationEl.textContent = course.duration;
    if (levelEl) levelEl.textContent = course.level;
    if (instructorEl) instructorEl.textContent = safeInstructor;
    if (videosTextEl) videosTextEl.textContent = `${videos.length} فيديو`;
    
    // تحديث قسم معلومات الدورة
    const videosCountEl = document.getElementById('course-videos-count');
    const totalHoursEl = document.getElementById('course-total-hours');
    const levelCardEl = document.getElementById('course-level');
    
    if (videosCountEl) videosCountEl.textContent = videos.length;
    if (totalHoursEl) totalHoursEl.textContent = course.duration.split(' ')[0]; // استخراج عدد الساعات فقط
    if (levelCardEl) levelCardEl.textContent = course.level;
    ...
    const pages = [
        '.home-page', '.my-courses-page', '.progress-page', 
        '.certificates-page', '.settings-page', '#course-details'
    ];
...
// إعداد مستمعي الأحداث للفيديوهات
function setupVideoEventListeners() {
    document.querySelectorAll('.watch-video-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (closeVideo) closeVideo.addEventListener('click', closeHandler);
            if (videoPlayer) videoPlayer.addEventListener('click', outsideHandler);
        });
    });
...
async function renderProgressPage() {
    ...
    if (!statsContainer || !coursesContainer) return;
...
function showMyCoursesPage() {
    ...
    const myCoursesPage = document.querySelector('.my-courses-page');
...
function showCertificatesPage() {
    ...
    if (certificatesPage) certificatesPage.style.display = 'block';
...
function showCertificateForm(courseId, courseTitle) {
    const certificateForm = document.getElementById('certificate-form-container');
    if (certificateForm) {
        certificateForm.style.display = 'flex';
        
        // حفظ بيانات الدورة مؤقتاً
        certificateForm.dataset.courseId = courseId;
        certificateForm.dataset.courseTitle = courseTitle;
    }
}
...
async function generateCertificate() {
    ...
    if (!studentNameInput || !certificateForm) return;
    
    const studentName = studentNameInput.value.trim();
    const courseId = certificateForm.dataset.courseId;
    const courseTitle = certificateForm.dataset.courseTitle;
...
function setupStatsAnimation() {
    ...
    if (!studentsCount || !coursesCount || !certificatesCount || !instructorsCount) return;
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Initializing Skillzoy Academy...');
    ...
    if (browseCoursesBtn) {
        browseCoursesBtn.addEventListener('click', showHomePage);
    }

    const browseMyCoursesBtn = document.getElementById('browse-my-courses-btn');
    if (browseMyCoursesBtn) {
        browseMyCoursesBtn.addEventListener('click', showMyCoursesPage);
    }
    
    // إعداد الشهادات
    const cancelCertificateBtn = document.getElementById('cancel-certificate');
    if (cancelCertificateBtn) {
        cancelCertificateBtn.addEventListener('click', closeCertificateForm);
    }
    ...
    const downloadPngBtn = document.getElementById('download-certificate-png');
    if (downloadPngBtn) {
    ...
    const downloadPdfBtn = document.getElementById('download-certificate-pdf');
    if (downloadPdfBtn) {
